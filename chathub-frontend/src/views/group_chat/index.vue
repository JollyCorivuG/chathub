<template>
    <div class="top">
        ...
        <div class="middle">
            {{groupInfo.name}} ({{groupInfo.peopleNum}})
        </div>
        ...
        <div v-for="(msg, index) in msgStore.msgList" :key="index" :id="'message' + index">
            ...
            <div v-if="msg.fromUser.id == userStore.userInfo.id" class="right">
                <div style="display: flex;flex-direction: column;align-items: flex-end">
                    <div style="margin-bottom: 3px;color: #888A97;">
                        <van-tag color="#FED231" v-if="userStore.userInfo.id == groupInfo.owner?.id">群主</van-tag>
                        <van-tag color="#A9B3C7" v-else>成员</van-tag>
            ...
            <div v-else class="left">
                <img :src="membersInfo.get(msg.fromUser.id)?.avatarUrl" alt="user-avatar" class="user-avatar">
                <div style="display: flex;flex-direction: column;align-items: flex-start">
                    <div style="margin-bottom: 3px;color: #888A97; margin-left: 2px">
                        <van-tag color="#FED231" v-if="msg.fromUser.id == groupInfo.owner?.id">群主</van-tag>
                        <van-tag color="#A9B3C7" v-else>成员</van-tag>
                        {{membersInfo.get(msg.fromUser.id)?.nickName}}
                    </div>
                    <div v-if="msg.message.msgType == MsgType.TEXT" class="message">
                        {{msg.message.body.content}}
                    </div>
...
import {GroupInfo, GroupInfoResponse} from "@/api/group/type.ts";
import {reqGroupInfo} from "@/api/group";
...
const groupInfo = ref<GroupInfo>({} as GroupInfo)
msgStore.roomId = Number(useRoute().query.roomId)

// 得到群组信息
// 开一个 map, key 为用户id, value 为用户信息
const membersInfo = ref<Map<number, UserInfo>>(new Map<number, UserInfo>())
const getGroupInfo = async () => {
    const groupId: number = Number(route.query.groupId)
    const resp: GroupInfoResponse = await reqGroupInfo(groupId)
    if (resp.statusCode != 0) {
        showNotify({type: 'danger', message: resp.statusMsg})
        return
    }
    groupInfo.value = resp.data
    // 将群组成员信息放到membersInfo中
    groupInfo.value.members?.forEach(item => {
        membersInfo.value.set(item.id, item)
    })
    membersInfo.value.set(groupInfo.value.owner?.id as number, groupInfo.value.owner as UserInfo)
}
getGroupInfo()
...
const getUserInfoById = async (id: number) => {
    // 如果 map 中没有, 就去请求
    if (!membersInfo.value.has(id)) {
        const resp: GetUserInfoResponse = await reqGetUserInfoById(id)
        if (resp.statusCode != 0) {
            showNotify({type: 'danger', message: resp.statusMsg})
            return {} as UserInfo
        }
        membersInfo.value.set(id, resp.data)
    }
    return membersInfo.value.get(id) as UserInfo
}
